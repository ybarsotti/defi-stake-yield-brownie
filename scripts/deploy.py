...
def deploy_token_farm_and_dapp_token(front_end_update=False):
    account = get_account()
    deployment_account_dict = {"from": account}
    dapp_token = DappToken.deploy(deployment_account_dict)
    token_farm = TokenFarm.deploy(dapp_token.address, deployment_account_dict,
                                  publish_source=
                                  config['networks'][network.show_active()][
                                      'verify'])
    tx = dapp_token.transfer(token_farm.address,
                             dapp_token.totalSupply() - KEPT_BALANCE,
                             deployment_account_dict)
    tx.wait(1)
...
def update_front_end():
    ...
    with open('brownie-config.yaml', 'r') as brownie_config:
        ...
        with open('./front_end/src/brownie-config.json',
                  'w') as brownie_config_json:
